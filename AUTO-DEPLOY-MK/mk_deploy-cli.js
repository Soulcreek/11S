...
async function uploadViaFTP(files, ftpConfig, remoteRoot, targetConfig) {
  ...
  try {
    ...
    // Add timestamp file to upload list - place it in the same directory as the project files
    const firstFile = files[0];
    const projectDir = firstFile ? path.dirname(firstFile.remotePath) : '/admin';
    
    const timestampFile = {
      localPath: timestampTempPath,
      relativePath: timestampFileName,
      remotePath: path.join(projectDir, timestampFileName).replace(/\\/g, '/'),
      webPath: path.join(path.dirname(firstFile?.webPath || '/admin'), timestampFileName).replace(/\\/g, '/'),
